...
public class ReverseOsmosisSim : MonoBehaviour
{
    ...
    void UpdateSimulationState(float pressure)
    {
        ...
        else if (pressure >= osmosisPressureThreshold) newState = SimState.LowPressureRO;
    ...
    {
        if (circulationMeasurements == null) return; // Essential check        
        ...
        for (int i = waterList.Count - 1; i >= 0; i--) // Iterate backwards for safe removal if needed (though recycling handles it now)
        {
            ...
            else
            {
                ...
                // Optional: If molecules store their own velocity, update it here:
                // MoleculeController mc = waterObj.GetComponent<MoleculeController>();
                // if (mc != null) mc.CurrentVelocity = velocity;
            }
        }
        ...
        foreach (GameObject contObj in allContaminants)
        {
            ...
            // 1. Calculate Velocity
            // Vector3 velocity = CalculateContaminantVelocity(); 
            ...
            if (contController.isStuck)
            {
                ...
                if (justUnstuck)
                {
                    // Give it a nudge away from membrane
                    velocity = -circulationMeasurements.ContamZoneAxis * contaminantUnstickNudgeSpeed;
                    // Apply only the nudge this frame
                    molTransform.position += velocity * Time.deltaTime;
                }
            ...
            molTransform.position = potentialPos;
            // Optional: Update velocity if stored
        }
    }
        ...
        ContaminantController contController, SimState state, float deltaTime) // Added controller, state, dt
    {
        ...
        Vector3 vecToPotential = potentialPos - axisReferencePoint; float distAlongAxis = Vector3.Dot(vecToPotential, primaryAxis); Vector3 pointOnAxis = axisReferencePoint + primaryAxis * distAlongAxis; float distFromAxisSq = (potentialPos - pointOnAxis).sqrMagnitude;
        if (distFromAxisSq > tubeRadSq) { Vector3 dirFromAxis = (potentialPos - pointOnAxis).normalized; potentialPos = pointOnAxis + dirFromAxis * cm.TubeRadius; Vector3 radialNormal = -dirFromAxis; velocity = Vector3.Reflect(velocity, radialNormal) * 0.8f; }
        ...
        if (distAlongAxis < 0) { pointOnAxis = flowStart + primaryAxis * Vector3.Dot(potentialPos - flowStart, primaryAxis); potentialPos = flowStart + (potentialPos - pointOnAxis); velocity = Vector3.Reflect(velocity, -contamAxis) * 0.8f; }
        ...
        if (distAlongAxis >= contamLength) // Hit or passed membrane position
        {
            ...
            // --- ADD LOG ---
            Debug.Log($"Membrane Hit: StickChance={currentStickChance:F3}, Random.value={Random.value:F3}");
            // -------------
            ...
            {
                // --- STICK ---
                // Snap position slightly right of membrane
                pointOnAxis = membrane + primaryAxis * Vector3.Dot(potentialPos - membrane, primaryAxis);
                potentialPos = membrane - contamAxis * 0.01f + (potentialPos - pointOnAxis); // Offset back right

                // Tell controller to get stuck
                float stickDuration = Random.Range(contaminantMinStickDuration, contaminantMaxStickDuration);
                if (state == SimState.OverPressure) stickDuration *= 1.5f; // Stick longer under pressure (optional)
                contController.GetStuck(stickDuration);

                velocity = Vector3.zero; // Stop movement
            }
            else
            {
                Debug.Log($"--> BOUNCING (Roll >= Chance)"); // IS THIS LOG APPEARING?
                // Snap position slightly right of membrane
                pointOnAxis = membrane + primaryAxis * Vector3.Dot(potentialPos - membrane, primaryAxis);
                potentialPos = membrane - contamAxis * 0.01f + (potentialPos - pointOnAxis); // Offset back right

                // Reflect velocity off membrane normal (points left = contamAxis)
                Vector3 originalVelocity = velocity; // Store before modification
                velocity = Vector3.Reflect(velocity, contamAxis) * 0.8f; // Modifies the 'ref' velocity
                Debug.Log($"Bounce Velocity: Original={originalVelocity.ToString("F3")}, Reflected={velocity.ToString("F3")}");
            }
        }
